...
import { Filter, AlertTriangle, Languages, Loader2, Box, List, Hash, Search } from "lucide-react"
...
type ViewMode = "model" | "charac" | "value"
...
export default function Translation() {
  ...
  // Missing translation counts per tab
  const missingModelCount = useMemo(() => {
    return productDatabase.filter(item => 
      (!item.nameFr && item.nameEn) || (!item.descriptionFr && item.descriptionEn)
    ).length
  }, [])

  const missingCharacCount = useMemo(() => {
    return characteristicsDatabase.filter(item => 
      (!item.nameFr && item.nameEn) || (!item.descriptionFr && item.descriptionEn)
    ).length
  }, [])

  const missingValueCount = useMemo(() => {
    return valuesDatabase.filter(item => 
      (!item.nameFr && item.nameEn) || (!item.descriptionFr && item.descriptionEn)
    ).length
  }, [])
  ...
  // Filtered data for charac view - characteristics are in a separate database
  const filteredCharacData = useMemo(() => {
    let data = characteristicsDatabase.map(item => {
      const translatedDesc = translationStatus.translatedItems.get(item.id)
      const translatedName = translationStatus.translatedNames?.get(item.id)
    ...
    return data
  }, [showMissingOnly, searchQuery, translationStatus.translatedItems, translationStatus.translatedNames])
  ...
  // Items with missing translations scoped to current viewMode
  const itemsMissingTranslations = useMemo(() => {
    if (viewMode === "model") {
      return productDatabase.filter(item => 
        (!item.nameFr && item.nameEn) || (!item.descriptionFr && item.descriptionEn)
      ) as UnifiedItem[]
    } else if (viewMode === "charac") {
      return characteristicsDatabase.filter(item => 
        (!item.nameFr && item.nameEn) || (!item.descriptionFr && item.descriptionEn)
      ) as UnifiedItem[]
    } else {
        ...
        <main className="flex-1 overflow-auto px-8 py-6 bg-background">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-4">
              <h2 className="text-lg font-semibold text-foreground">
                {viewMode === "model" 
                  ...
                  : viewMode === "charac"
                  ? `Caractéristiques (${filteredCharacData.length} elements)`
                  : `Valeurs (${filteredValueFirstData.length} valeurs)`
                }
              </h2>
              {/* View Mode Toggle */}
              <div className="flex items-center bg-muted rounded-lg p-1">
                ...
                >
                  ...
                  Model
                  {missingModelCount > 0 && (
                    <span className="ml-1 px-1.5 py-0.5 text-xs font-medium rounded-full bg-amber-100 text-amber-700">
                      {missingModelCount}
                    </span>
                  )}
                </button>
                <button
                  onClick={() => setViewMode("charac")}
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-md transition-colors ${
                    viewMode === "charac"
                      ? "bg-card text-foreground shadow-sm"
                      : "text-muted-foreground hover:text-foreground"
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                Tout traduire ({missingModelCount + missingCharacCount + missingValueCount})
              </button>
              <button
                onClick={() => {
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1 min-w-0 overflow-auto">
              {viewMode === "model" || viewMode === "charac" ? (
                <TranslationTable 
                  data={viewMode === "charac" ? (filteredCharacData as unknown as ProductItem[]) : filteredData} 
                  selectedIds={selectedIds} 
